...
void *alarm_thread(void *arg) {
  ...
  while (1) {
    ...
    /*
     * Unlock the mutex before waiting, so that the main thread can lock it to
     * insert a new alarm request. If the sleep_time is 0, then call
     * sched_yield, giving the main thread a chance to run if it has been
     * readied by user input, without delaying the message if there's no input.
     */
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
     ...
     * to 64 characters, consisting of up to 64 characters separated from the
    ...
    /*
     * Insert the new alarm into the list of alarms, sorted, by expiration
     * time.
     */
