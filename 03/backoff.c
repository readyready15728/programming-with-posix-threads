...
/*
 * Initialize a static array of three mutexes.
 */
...
/*
 * This is a thread start routine that locks all mutexes in order, to ensure a
 * conflict with lock_reverse, which does the opposite.
 */
...
/*
 * This is a thread start routine that locks all mutexes in reverse order, to
 * ensure a conflict with lock_forward, which does the opposite.
 */
...
int main(int argc, char *argv[]) {
  ...
  /* 
   * If the first argument is absent, or non-zero, a backoff algorithm will be
   * used to avoid deadlock. If the first argument is zero, the programm will
   ...
   * On some systems, especially uniprocessors, one thread may complete before
   * the other has a chance to run, and you won't see a deadlock or backoffs.
   * In that case, try running with the argument set to a positive number to
   * cause the threads to call sched_yield() at each lock; or, to make it even
   * more obvious, set to a negative number to cause the threads to call
   * sleep(1) instead.
   */
